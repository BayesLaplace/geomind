...
> ⚠️ 强烈建议使用环境变量,**不要**把真实 Key 写进 `config.py` 后再提交到 git。
...
或参考仓库根目录的 [.env.example](.env.example),复制为 `.env`(已被 `.gitignore` 忽略)后填入自己的 Key,然后用 `python-dotenv` 之类的工具加载,或在启动前手动 `source` 进环境。

**方式 B:直接改 config.py(仅本地试用)**

打开 [config.py](config.py),把 `API_KEY` 行的占位符替换为你自己的 Key:

```python
API_KEY = os.environ.get("DASHSCOPE_API_KEY", "sk-xxxxxxxxxxxxxxxx")
#                                              ^^^^^^^^^^^^^^^^^^^^ 替换这里
```

修改后请务必把 `config.py` 也加入 `.gitignore`,避免 Key 被一并推送。
...
- **初中**:三角形全等 / 相似、平行四边形、中位线、切线长定理
- **高中**:立体几何投影、圆锥曲线性质、向量法证明
- **课外辅导 / 自学**:学生作业卡壳时随时调用,看每步图自查
